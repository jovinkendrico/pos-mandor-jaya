...
class LargeDataSeeder extends Seeder
{
    ...
    private function confirmSales(): void
    {
        ...
        $batchSize = 100; // Batch size for confirmation
        ...
        Sale::where('status', 'pending')
            ...
            ->chunk($batchSize, function ($sales) use ($stockService, &$processed, &$failed, $totalSales) {
                ...
                if ($processed % 1000 === 0 || $processed + $failed >= $totalSales) {
    ...
    private function createSalePayments($banks): void
    {
        ...
        $batchSize = 100;
        $totalPayments = min(10000, $sales->count()); // Max 10000 payments
        $processed = 0;
        ...
        foreach ($sales->take($totalPayments) as $sale) {
            ...
            // Create payment item
            DB::table('sale_payment_items')->insert([
            ...
            if ($receivableAccount) {
                $dateStr = $paymentDate->format('Ymd');
                ...
                DB::table('cash_ins')->insert([
                    ...
                    'cash_in_date' => $paymentDate,
                    'bank_id' => $bankId,
                    'chart_of_account_id' => $receivableAccount->id,
                    'amount' => $paymentAmount,
                    'description' => "Pembayaran Penjualan #{$paymentNumber}",
                    'status' => 'posted',
            ...
            // Update bank balance
            DB::table('banks')->where('id', $bankId)->increment('balance', $paymentAmount);
    ...
    private function createPurchasePayments($banks): void
    {
        ...
        $paymentMethods = ['cash', 'transfer', 'giro', 'cek', 'other'];
        $batchSize = 100;
        $totalPayments = min(10000, $purchases->count()); // Max 10000 payments
        $processed = 0;
        ...
        foreach ($purchases->take($totalPayments) as $purchase) {
            if ($processed >= $totalPayments) {
                break;
            }
            ...
            // Create payment item
            DB::table('purchase_payment_items')->insert([
            ...
            if ($payableAccount) {
                ...
                DB::table('cash_outs')->insert([
                    ...
                    'cash_out_date' => $paymentDate,
                    'bank_id' => $bankId,
                    'chart_of_account_id' => $payableAccount->id,
                    'amount' => $paymentAmount,
                    'description' => "Pembayaran Pembelian #{$paymentNumber}",
                    'status' => 'posted',
            ...
            // Update bank balance
            DB::table('banks')->where('id', $bankId)->decrement('balance', $paymentAmount);
    ...
    private function createCashInOut($banks): void
    {
        ...
        for ($i = 0; $i < $totalCashIn; $i++) {
            ...
            $bankId = $bankIds[array_rand($bankIds)];
            $coaId = $incomeAccounts[array_rand($incomeAccounts)] ?? null;
            ...
            // Update bank balance
            DB::table('banks')->where('id', $bankId)->increment('balance', $amount);
            
            if (($i + 1) % 1000 === 0) {
                $this->command->info("Created cash in: " . ($i + 1) . " / {$totalCashIn}");
            }
        }
        
        // Create Cash Out
        $this->command->info('Creating cash out transactions...');
        for ($i = 0; $i < $totalCashOut; $i++) {
            $date = Carbon::now()->subDays(rand(0, 365));
            $dateStr = $date->format('Ymd');
            $lastCashOut = DB::table('cash_outs')
                ->where('cash_out_number', 'like', 'CO' . $dateStr . '%')
            ...
            $bankId = $bankIds[array_rand($bankIds)];
            $coaId = $expenseAccounts[array_rand($expenseAccounts)] ?? null;
            ...
            // Update bank balance
            DB::table('banks')->where('id', $bankId)->decrement('balance', $amount);
            
            if (($i + 1) % 1000 === 0) {
                $this->command->info("Created cash out: " . ($i + 1) . " / {$totalCashOut}");
            }
        }
    }
}
