...
export default function ItemForm({ item, isModalOpen, onOpenChange }: ItemFormProps) {
    const form = useForm<{
        code: string;
        name: string;
        base_uom: string;
        ...
        if (isModalOpen) {
            if (item) {
                ...
                const normalizedUoms = (item.uoms || []).map((uom) => ({
                    ...uom,
                    conversion_value: typeof uom.conversion_value === 'string'
    ...
    const handleUomChange = (index: number, field: keyof ItemUom, value: string | boolean | number) => {
        const newUoms = [...uoms];
        ...
        if (field === 'conversion_value') {
            ...
            newUoms[index] = { ...newUoms[index], [field]: isNaN(numValue) ? 1 : numValue };
        } else {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const hasEmptyUom = uoms.some((uom) => {
            const convValue = typeof uom.conversion_value === 'string'
                ? parseInt(uom.conversion_value)
                : uom.conversion_value;
            return !uom.uom_name.trim() || !uom.conversion_value || convValue < 1 || !uom.price;
        });
        if (hasEmptyUom) {
        ...
        const normalizedUoms = uoms.map((uom) => ({
            ...
            conversion_value: typeof uom.conversion_value === 'string'
                ? parseInt(uom.conversion_value)
                : uom.conversion_value,
            price: typeof uom.price === 'string' ? uom.price : String(uom.price),
            is_base: uom.is_base,
        }));
    ...
    return (
        <Dialog open={isModalOpen} onOpenChange={onOpenChange}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                    <Label htmlFor="stock">Stock (dalam Base UOM)</Label>
                    <Input
                        id="stock"
                        type="number"
                        step="0.01"
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Tambahkan satuan untuk barang ini. Klik badge "Base" untuk set UOM dasar.
                        </p>
                    </div>
                    <Button type="button" onClick={handleAddUom} size="sm" variant="outline">
                ...
                <div className="space-y-3 border rounded-lg p-4">
                    {uoms.map((uom, index) => (
                        <div
                            key={index}
                            className="grid gap-3 md:grid-cols-[2fr_1.5fr_1.5fr_auto_auto] items-end p-3 border rounded-lg bg-muted/50"
                        >
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    value={uom.uom_name}
                                    onChange={(e) =>
                                        handleUomChange(index, 'uom_name', e.target.value)
                                    }
                            ...
                            <div className="space-y-2">
                                <Label htmlFor={`conversion_${index}`}>
                                    Konversi ke Base <span className="text-red-500">*</span>
                                </Label>
                                <Input
                                    ...
                                    onChange={(e) =>
                                        handleUomChange(index, 'conversion_value', e.target.value)
                                    }
                                    placeholder="1"
                                    required
                                    disabled={form.processing || uom.is_base}
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <Badge
                                    ...
                                    onClick={() => handleSetBaseUom(index)}
                                >
                            ...
                            <Button
                                ...
                                disabled={form.processing || uoms.length === 1}
                            >
                ...
                <div className="text-sm text-muted-foreground space-y-1 bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="font-semibold text-blue-900">💡 <strong>Panduan Multi-UOM:</strong></p>
                    <ul className="list-disc list-inside space-y-1 ml-4 text-blue-800">
                        <li><strong>Base UOM = Satuan TERKECIL</strong> (bukan terbesar)</li>
                        <li>Contoh: Base UOM = <strong>PCS</strong> (1 pcs = unit terkecil)</li>
            ...
            <div className="flex justify-end gap-3">
                ...
                <Button type="submit" disabled={form.processing}>
                    {form.processing ? 'Menyimpan...' : item ? 'Update' : 'Simpan'}
                </Button>
            </div>
        </form>
            </DialogContent>
