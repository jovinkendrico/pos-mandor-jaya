...
export default function UserForm({ user, isModalOpen, onOpenChange, roles }: UserFormProps) {
    const form = useForm({
        name: '',
        email: '',
        password: '',
    ...
    useEffect(() => {
        if (user) {
            ...
            const userRoleIds = user.roles && Array.isArray(user.roles) ? user.roles.map((role: any) => role.id) : [];
        ...
        <Dialog open={isModalOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {user ? 'Perbarui informasi user di bawah.' : 'Isi data detail untuk menambahkan user baru'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col space-y-2">
                    <div className="flex flex-col space-y-4 pb-4">
                        ...
                        <div className="flex flex-row gap-4">
                            <div className="w-1/2">
                                ...
                                {user && <p className="mt-1 text-sm text-muted-foreground">Kosongkan jika tidak diubah</p>}
                            </div>
                            ...
                            <div className="w-1/2">
                                ...
                                />
                                {form.errors.password_confirmation && <InputError message={form.errors.password_confirmation} />}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                                {roles.map((role) => (
                                    <Badge
                                        key={role.id}
                                        variant={form.data.roles?.includes(role.id) ? 'default' : 'outline'}
                            ...
                            {form.errors.roles && <InputError message={form.errors.roles} />}
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={form.processing}>
                            {form.processing ? 'Saving...' : user ? 'Update User' : 'Tambah User'}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
