...
export default function PurchaseReturnShow({ return: returnData }: PageProps) {
    ...
    const handleConfirm = () => {
        if (confirm('Konfirmasi retur? Stock akan berkurang dan data tidak bisa diedit lagi.')) {
            router.post(`/purchase-returns/${returnData.id}/confirm`, {}, {
                onSuccess: () => toast.success('Retur pembelian dikonfirmasi'),
    ...
    const handleUnconfirm = () => {
        if (confirm('Batalkan konfirmasi? Stock akan dikembalikan.')) {
            router.post(`/purchase-returns/${returnData.id}/unconfirm`, {}, {
        ...
        <>
            <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <PageTitle title={`Retur Beli ${returnData.return_number}`} />
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-2">
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">No. Pembelian:</span>
                                <span className="font-medium font-mono">{returnData.purchase.purchase_number}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Supplier:</span>
                                <span className="font-medium">{returnData.purchase.supplier?.name || '-'}</span>
                            </div>
                            <div className="flex justify-between">
