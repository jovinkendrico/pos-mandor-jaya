...
<body>
    ...
    <div class="header-wrapper">
        <div class="info-section">
            <div class="info-row">
                ...
                <span class="info-value">: {{ $saleReturn->return_number }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Tanggal Retur</span>
                <span class="info-value">: {{ \Carbon\Carbon::parse($saleReturn->return_date)->format('d-m-Y') }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">No. Penjualan</span>
                <span class="info-value">: {{ $saleReturn->sale->sale_number ?? '-' }}</span>
            </div>
        ...
        <div class="info-section" style="margin-top: 8px; vertical-align: top;">
            ...
            <div class="info-row">
                <div class="info-value">{{ $saleReturn->sale->customer->name ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-value">{{ $saleReturn->sale->customer->city->name ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-value">{{ $saleReturn->sale->customer->phone_number ?? '-' }}</div>
            </div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
             @php
                $maxRows = 12;
                $detailsCount = count($saleReturn->details);
             @endphp
             ...
             <tr>
                <td colspan="3" class="text-left" style="border-top: 1px dashed #000;">Terbilang : <i style="text-transform: capitalize;">
                    {{ Terbilang::make($saleReturn->total_amount) }} Rupiah
                </i>
                </td>
                <td style="border-top: 1px dashed #000;">
                    Total: 
                </td>
                <td style="border-left: none; border-top: 1px dashed #000;" class="text-right total">Rp. {{ number_format($saleReturn->total_amount, 0, ',', '.') }}</td>
             </tr>
        </tbody>
    </table>
