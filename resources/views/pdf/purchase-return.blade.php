...
<body>
    ...
    <div class="header-wrapper">
        <div class="info-section">
            <div class="info-row">
                ...
                <span class="info-value">: {{ $purchaseReturn->return_number }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Tanggal Retur</span>
                <span class="info-value">: {{ \Carbon\Carbon::parse($purchaseReturn->return_date)->format('d-m-Y') }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">No. Pembelian</span>
                <span class="info-value">: {{ $purchaseReturn->purchase->purchase_number ?? '-' }}</span>
            </div>
        ...
        <div class="info-section" style="margin-top: 5cm; vertical-align: top;">
            ...
            <div class="info-row">
                <div class="info-value">{{ $purchaseReturn->purchase->supplier->name ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-value">{{ $purchaseReturn->purchase->supplier->city->name ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-value">{{ $purchaseReturn->purchase->supplier->phone_number ?? '-' }}</div>
            </div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
             @php
                $maxRows = 12;
                $detailsCount = count($purchaseReturn->details);
             @endphp
             ...
             <tr>
                <td colspan="3" class="text-left" style="border-top: 1px dashed #000;">Terbilang : <i style="text-transform: capitalize;">
                    {{ Terbilang::make($purchaseReturn->total_amount) }} Rupiah
                </i>
                </td>
                <td style="border-top: 1px dashed #000;">
                    Total: 
                </td>
                <td style="border-left: none; border-top: 1px dashed #000;" class="text-right total">Rp. {{ number_format($purchaseReturn->total_amount, 0, ',', '.') }}</td>
             </tr>
        </tbody>
    </table>
